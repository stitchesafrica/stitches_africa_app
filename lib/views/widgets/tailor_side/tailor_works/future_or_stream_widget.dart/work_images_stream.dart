...
import 'package:stitches_africa/views/widgets/tailor_side/tailor_works/tailor_side_images_widget.dart';
import 'package:toastification/toastification.dart';

class WorkImagesStream extends ConsumerStatefulWidget {
  final Stream<TailorWorkModel?> getTailorModel;
  const WorkImagesStream({super.key, required this.getTailorModel});

  @override
  ConsumerState<WorkImagesStream> createState() => _WorkImagesStreamState();
}

class _WorkImagesStreamState extends ConsumerState<WorkImagesStream> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<TailorWorkModel?>(
        ...
        builder: (context, snapshot) {
          ...
          return SizedBox(
            ...
            child: ListView.builder(
                ...
                itemCount: tailorModel.images.length + 1,
                itemBuilder: (context, index) {
                  if (index + 1 != tailorModel.images.length + 1) {
                    return TailorSideImagesWidget(
                      imageUrl: tailorModel.images[index],
                      productId: tailorModel.productId,
                    );
                  } else {
                    return GestureDetector(
                      onTap: () async {
                        await uploadImagesHandler().then((_) async {
                          await firebaseFirestoreFunctions.addImagesToWorks(
                              tailorModel.productId, mediaPaths);
