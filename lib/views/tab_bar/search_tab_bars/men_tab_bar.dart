...
class MenTabBar extends ConsumerWidget {
  final List<String> list;
  MenTabBar({super.key, required this.list});
  ...
  Future<List<AlgoliaObjectSnapshot>> queryOperation(
    ...
    try {
      ...
      query = query.facetFilter('category:men');
  ...
  Future<List<String>> getRecentSearchesForMen() async {
    Box box = Hive.box('user_preferences');
    Map<String, dynamic> userPreferences =
        Map<String, dynamic>.from(await box.get('user') ?? {});

    return List<String>.from(userPreferences['recentSearchesMen'] ?? []);
  }

  Stream<List<RecentlyViewedModel>> getRecentlyViewedItemsForMenTab() {
  ...
  Widget _buildIfSearchBoxIsTapped() {
    return BuildIfSearchBoxIsTappedStreamWidget(
        getRecentlyViewedItemsForWomenTab: getRecentlyViewedItemsForMenTab(),
        getRecentSearchesForWomen: getRecentSearchesForMen());
  }
