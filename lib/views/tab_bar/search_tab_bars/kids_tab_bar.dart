...
class KidsTabBar extends ConsumerWidget {
  final List<String> list;
  KidsTabBar({super.key, required this.list});
  ...
  Future<List<AlgoliaObjectSnapshot>> queryOperation(
    ...
    try {
      ...
      query = query.facetFilter('category:kids');
  ...
  Future<List<String>> getRecentSearchesForKids() async {
    Box box = Hive.box('user_preferences');
    Map<String, dynamic> userPreferences =
        Map<String, dynamic>.from(await box.get('user') ?? {});

    return List<String>.from(userPreferences['recentSearchesKids'] ?? []);
  }

  Stream<List<RecentlyViewedModel>> getRecentlyViewedItemsForKidsTab() {
  ...
  Widget _buildIfSearchBoxIsTapped() {
    return BuildIfSearchBoxIsTappedStreamWidget(
        getRecentlyViewedItemsForWomenTab: getRecentlyViewedItemsForKidsTab(),
        getRecentSearchesForWomen: getRecentSearchesForKids());
  }
